 ...
 *
 * This test enables ONLY the Attention kernel for HAT dispatch while keeping all other kernels
 * in plain Java mode. Note that the base Attention implementation already uses HAT dispatch
 * for both computeScores and computeValues, so this test validates the explicit HAT version.
 *
 * Attention characteristics:
 * - Two-step HAT dispatch: compute scores (Q*K^T), compute values (Scores*V)
 * - First step parallelized over sequence length
 * - Second step parallelized over head dimension
 * - Core component of transformer self-attention mechanism
 * - Depends on Softmax (applied between the two steps)
 *
 * Success criteria:
...
public class ChatIntegrationTestWithAttentionHAT {
    ...
    public void testChatWithAttentionHAT() throws IOException {
        ...
        // Create factory with ONLY Attention HAT enabled
