 ...
 *
 * This test enables ONLY the Softmax kernel for HAT dispatch while keeping all other kernels
 * in plain Java mode. Note that Softmax already uses HAT for the normalize step in the base
 * implementation, so this test validates that the explicit HAT version works correctly.
 *
 * Softmax characteristics:
 * - Three-step operation: find max, compute exp/sum, normalize
 * - Only the normalize step uses HAT dispatch (element-wise parallelizable)
 * - Max and sum steps use plain Java (reduction operations)
 * - Critical for attention mechanism (applied to attention scores)
 *
 * Success criteria:
...
public class ChatIntegrationTestWithSoftmaxHAT {
    ...
    public void testChatWithSoftmaxHAT() throws IOException {
        ...
        // Create factory with ONLY Softmax HAT enabled
