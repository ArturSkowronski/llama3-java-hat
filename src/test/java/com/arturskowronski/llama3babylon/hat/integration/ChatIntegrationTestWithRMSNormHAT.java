 ...
 *
 * This test enables ONLY the RMSNorm kernel for HAT dispatch while keeping all other kernels
 * in plain Java mode. RMSNorm is applied extensively (33 times per token: 2 per layer + 1 final).
 *
 * RMSNorm characteristics:
 * - Two-step operation: compute sum of squares, then normalize
 * - Sum computation uses plain Java (reduction operation)
 * - Normalization uses HAT dispatch (element-wise parallelizable)
 * - Critical for layer normalization in transformer architecture
 *
 * Success criteria:
...
public class ChatIntegrationTestWithRMSNormHAT {
    ...
    public void testChatWithRMSNormHAT() throws IOException {
        Path modelPath = Paths.get(System.getenv("LLAMA_FP16_PATH"));

        // Create factory with ONLY RMSNorm HAT enabled
        HybridKernelFactory factory = new HybridKernelFactory(
            Set.of(HybridKernelFactory.KernelType.RMSNORM)
        );
        ...
        System.out.println("=== Model Response (RMSNorm HAT) ===");
