...
See `GEMVBufferCachingBugTest.java` for a self-contained test suite that demonstrates the bug and its workaround.

The GEMV kernel under test:
...
`GEMVBufferCachingBugTest.java` contains 5 tests:

| Test | Purpose |
|------|---------|
| `testSmallMatrixAlone` | Baseline — single small GEMV works |
| `testLargeMatrixAlone` | Baseline — single large GEMV works |
| `testSmallThenLarge_FAILS_due_to_buffer_caching` | **BUG** — small then large throws IndexOutOfBoundsException |
| `testSeparateInstances_STILL_FAILS` | **BUG** — separate GEMV instances sharing Accelerator still fail |
| `testWorkaround_largestFirst` | **WORKAROUND** — calling largest matrix first makes all subsequent calls succeed |
